...
def update_sum_formulas_in_row(fb_sheet, date_row):
    ...
    start_col_letter = 'G'  # Starting from column G
    step = 6  # Step size
    total_cols = fb_sheet.col_count  # Total number of columns in the sheet

    # Iterate through the cells B, C, D
    for i in range(2, 5):  # B(2), C(3), D(4)
        # Calculate the start column letter for this iteration
        calculated_col_letter = column_index_to_string(column_letter_to_index('G') + (i - 2))
        sum_formula = create_sum_formula(calculated_col_letter, date_row, step, total_cols)
        cell_to_update = f'{chr(64 + i)}{date_row}'  # Convert 2 to B, 3 to C, and 4 to D
        fb_sheet.update(cell_to_update, sum_formula, value_input_option='USER_ENTERED')
        print(f'Updated cell {cell_to_update} with formula: {sum_formula}')
    
    ad_spend_total = gspread.utils.rowcol_to_a1(date_row, 2)
    leads_total=gspread.utils.rowcol_to_a1(date_row, 3)
    comments_total=gspread.utils.rowcol_to_a1(date_row, 4)
    for i in range(5, 7):
        calculated_col_letter =column_index_to_string(i)
        cell_to_update = f'{calculated_col_letter}{date_row}'  # Convert 5 to E, etc
        sum_formula=''
        if i==5:
        ...
        fb_sheet.update(cell_to_update, sum_formula, value_input_option='USER_ENTERED')
        print(f'percentage i={i} Updated cell {cell_to_update} with formula: {sum_formula}')
